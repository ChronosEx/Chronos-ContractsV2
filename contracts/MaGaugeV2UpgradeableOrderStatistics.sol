...
contract MaGaugeV2UpgradeableOrderStatistics is
    OwnableUpgradeable,
    ...
    ERC721EnumerableUpgradeable
{
    ...
    // there might be a more elegant solution to deleting the old tree and creating a new one
    uint private MAIN;
    uint private REPLACEMENT;
    // red-black tree used for position entries sorting
    mapping(uint => HitchensOrderStatisticsTreeLib.Tree) _preLimitMaxTimestampsTree;
    mapping(uint => EnumerableSet.UintSet) private _maxTimestampToPositionIds;
    ...
    function initialize(
        ...
        MAIN = 0;                               // initial index of the main tree
        REPLACEMENT = 1;                        // index of the replacement tree
    ...
    function split(
        ...
        uint maxLimitEntry = positionEntry + maturityIncrement * 2;
        ...
        for (uint i; i < weights.length; i++) {
            ...
            if (preLimit) {
                _maxTimestampToPositionIds[maxLimitEntry].add(_newMANFTId);
            }
        }
        ...
        if (preLimit) {
            ...
            _maxTimestampToPositionIds[maxLimitEntry].remove(_maNFTId);
    ...
    */
    ...
    function _updateTotalWeight() private {
        // if entries below < 50%, go from start and delete entries
        // if entries below >= 50%, go from end and add to new tree
        uint median = _preLimitMaxTimestampsTree[MAIN].median();
        if (median > block.timestamp) {
            _updateWeightWithRemovalFromStart();
        } else {
            _updateWeightWithAdditionFromEnd();
        }
        _lastTotalWeightUpdateTime = block.timestamp;
    }

    function _updateWeightWithAdditionFromEnd() private {
        uint newWeightIncrement = 0;
        uint newLpTotalSupplyPreLimit = 0;
        uint totalWeightPreLimit = 0;
        uint currentEntry = _preLimitMaxTimestampsTree[MAIN].last();

        while (currentEntry >= block.timestamp) {
            if (currentEntry == 0) {
                break;
            }

            uint nextEntry = _preLimitMaxTimestampsTree[MAIN].prev(currentEntry);

            for (uint i; i < _maxTimestampToPositionIds[currentEntry].length(); i++) {
                uint currentId = _maxTimestampToPositionIds[currentEntry].at(i);
                newLpTotalSupplyPreLimit += _lpBalances[currentId];
                uint weight = _lpBalances[currentId] * _maturityMultiplier(currentId);
                totalWeightPreLimit += _lpBalances[currentId] * _maturityMultiplier(currentId);
                newWeightIncrement += weight - _lpBalances[currentId] * MATURITY_PRECISION;

                _preLimitMaxTimestampsTree[REPLACEMENT].insert(0, currentEntry);
            }

            currentEntry = nextEntry;
        }

        _lpTotalSupplyPostLimit = lpTotalSupply() - newLpTotalSupplyPreLimit;
        _weightIncrement = newWeightIncrement;
        _lpTotalSupplyPreLimit = newLpTotalSupplyPreLimit;

        delete _preLimitMaxTimestampsTree[MAIN];
        MAIN = REPLACEMENT;
        REPLACEMENT += 1;

        _totalWeight = totalWeightPreLimit + _lpTotalSupplyPostLimit * MATURITY_PRECISION * 3;
    }

    function _updateWeightWithRemovalFromStart() private {
        ...
        uint currentEntry = _preLimitMaxTimestampsTree[MAIN].first();
        // uint entriesBelow = _preLimitMaxTimestampsTree.below(block.timestamp);
        while (currentEntry <= block.timestamp) {
            if (currentEntry == 0) {
                break;
            }

            uint nextEntry = _preLimitMaxTimestampsTree[MAIN].next(currentEntry);
            ...
            _preLimitMaxTimestampsTree[MAIN].remove(0, currentEntry);

            delete _maxTimestampToPositionIds[currentEntry];
    ...
    function _addToPosition(
        ...
        if (multiplier < MATURITY_PRECISION * 3) {
            ...
            // add new entry
            _preLimitMaxTimestampsTree[MAIN].insert(0, entry + 2 * maturityIncrement);
            _maxTimestampToPositionIds[entry + 2 * maturityIncrement].add(id);
        } else {
    ...
    function _removeFromPosition(
        ...
        if (multiplier < MATURITY_PRECISION * 3) {
            ...
            // remove old entry
            _maxTimestampToPositionIds[entry + 2 * maturityIncrement].remove(id);
            // remove entry from tree if it's the only id
            if (_maxTimestampToPositionIds[entry + 2 * maturityIncrement].length() == 0) {
                _preLimitMaxTimestampsTree[MAIN].remove(0, entry + 2 * maturityIncrement);
                delete _maxTimestampToPositionIds[entry + 2 * maturityIncrement];
            }
        } else {
            _lpTotalSupplyPostLimit -= amount;
        }
    ...
    // is slightly > total weight. the following processing is to avoid underflow
    function _reduceWeightIncrement(uint amount) internal returns (uint reduction) {
